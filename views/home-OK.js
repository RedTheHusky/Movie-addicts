...
function displayMovies() {
  ...
  console.log("display movies.items=", movies.items);
	...
	if(Worker&&hWorker)
		hWorker.postMessage({movies_items:movies.items,movies_pagination:movies.pagination,timer4Check:"start"});
};
...
function doStats(){
	if(Worker&&hWorker)
		hWorker.postMessage({get:"stats"});
}
if(Worker&&hWorker)
hWorker.onmessage = function(event) {
	console.groupCollapsed('hWorker.onmessage');
